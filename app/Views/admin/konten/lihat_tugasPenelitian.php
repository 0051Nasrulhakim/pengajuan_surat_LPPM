...
<?= $this->section('content'); ?>
    <div class="page">
        <div class="form">
            ...
            <?php
                $sukes = session('update_tugas_penelitian');
                if($sukes != null ){
                    echo '<div class="alert alert-success">'.$sukes.'</div>';
                }
                
                $error = session('pesan_error_nomor_surat_tugas_penelitian');
                if($error != null ){
            ...
            <?php foreach($data as $d):?>
                <form action="">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <textarea name="keperluan" id="keprluan" rows="4" class="form-control" readonly><?= $d['keperluan']?></textarea>
                ...
                <div class="lokasi_tombol">
                    <a href="<?=base_url()?>/pdf_output/preview_tugasPenelitian/<?=$d['id'];?>?nomor_surat=<?=$d['nomor_surat']?>"><button type="submit" class="tombol tombol_biru"><i class="fa fa-eye" aria-hidden="true">Download</i></button></a>
                    <a href="<?=base_url()?>/admin/edit_tugasPenelitian/<?=$d['id'];?>"><button class="tombol tombol_kuning"><i class="fa fa-pencil" aria-hidden="true">Edit</i></button></a>
                </div>
            <?php endforeach;?>
        </div>
    </div>
